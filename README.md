...
- **编辑**：单击选格、拖选、双击或直接键入；公式栏；中文输入法；撤销 / 重做；格式刷；填充柄（序列填充）；单元格备注
- **公式**：`SUM` `AVERAGE` `MAX` `MIN` `COUNT` `COUNTA`、逻辑（`IF` `AND` `OR` 等）、文本（`CONCAT` `LEFT` `TEXT` 等）、日期、查找（`VLOOKUP` `XLOOKUP`）、数学、条件汇总；支持 `A1`、`A1:B5`、跨表 `Sheet1!A1`、`&` 连接；编辑公式时可点选单元格插入引用并高亮引用区
- **样式**：字体、字号、加粗 / 倾斜 / 下划线 / 删除线、颜色、填充、对齐、边框、换行、数字格式（常规 / 数值 / 货币 / 百分比 / 日期等，可增减小数位）
- **结构**：插删行列、在选区下方追加行、拖拽改行高列宽、合并 / 拆分
- **浏览**：查找 / 替换（值、公式、批注，当前表或整个工作簿）；状态栏显示选区、平均值、计数、求和
- **筛选与冻结**：自动筛选（排序、勾选值、按标记筛选）；冻结窗格
- **标记**：优先级、形状、结论标记；从 Excel 读写可编辑区域、勾选和下拉 / 开关控件
- **打印**：打印预览，按使用区域分页
- **多表**：底栏新增、切换、双击改名；右键删除（确认）、左移 / 右移、拖拽重排
